...
export class AuthClient {
  ...
  async login(credentials: AuthCredentials): Promise<UserSession> {
    try {
      const response = await axios.post(`${this.config.authUrl}/login`, {
      ...
      if (response.status === 200 && response.data.user && response.data.sessionToken) {
      ...
      if (axios.isAxiosError(error)) {
        ...
        throw new Error(`認証エラー: ${error.response?.data?.error || error.message}`);
      }
  ...
  async logout(sessionToken: string): Promise<void> {
    try {
      await axios.post(`${this.config.authUrl}/logout`, {}, {
        headers: {
          'Cookie': `authjs.session-token=${sessionToken}`
        }
      });
