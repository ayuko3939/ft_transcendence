...
const generateDummyMatches = (count: number) => {
  ...
  return Array.from({ length: count }, (_, i) => {
    ...
    const playerScore = isWin
      ? 10 + Math.floor(Math.random() * 5)
      : 5 + Math.floor(Math.random() * 5);
    const opponentScore = isWin
      ? Math.floor(Math.random() * 5)
      : playerScore + 1 + Math.floor(Math.random() * 5);
      ...
      <div className={styles.matchList}>
        {matches.map((match) => (
          <div key={match.id} className={styles.matchItem}>
            ...
            <div
              className={`${styles.matchScore} ${match.result === "win" ? styles.winScore : styles.loseScore}`}
            >
